...
def _main_content():
    ...
    2b. HERANÇA DE KIT — quando o usuário disser "essas são [kit]" / "as que eu vou falar são [kit]" / "são [kit]" no início ou separado por vírgula/traço:
       → aplique esse kit a TODOS os itens seguintes. O kit NÃO é um modelo de aparelho — é o tipo de produto.
       → nome_produto de cada item = o MODELO DO APARELHO que vem a seguir
       → NUNCA use o nome do kit (ex: "Space") como nome_produto
       Ex: "As que eu vou falar agora são Space, iPhone 17, 5, iPhone 15, 5, iPhone 16, 5"
       → iPhone 17 | kit="space" | quantidade_fixa=5
    ...
    Exemplo E — herança de kit (produto onde modelo=variação):
    "As que eu vou falar agora são Space, iPhone 17, 5, iPhone 15, 5, iPhone 16, 5, iPhone 7 Plus, 4, iPhone 16E, 5"
    → iPhone 17 | kit="space" | quantidade_fixa=5          ← "Space" é o KIT herdado por todos
    → iPhone 15 | kit="space" | quantidade_fixa=5
    → iPhone 16 | kit="space" | quantidade_fixa=5
    ...
    if _pg == "balanco_lojas":
        ...
        import os as _os_v2
        _V2_URL = _os_v2.environ.get("V2_URL", "http://localhost:8000").rstrip("/")
        _V2_HEADERS = {"x-internal-token": _os_v2.environ.get("INTERNAL_TOKEN", "")}
